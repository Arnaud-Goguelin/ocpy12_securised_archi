...
class TestGetAllBySalesperson:
    def test_returns_customers_for_salesperson(self, db_session):
        salesperson = UserDBFactory(role=Roles.SALES)
        CustomerDBFactory(salesperson=salesperson, salesperson_id=salesperson.id)
        CustomerDBFactory()
        result = CustomerService.get_all_by_salesperson(salesperson, db_session)
        assert len(result) == 1
        assert result[0].salesperson_id == salesperson.id

    def test_returns_empty_list_when_no_customers(self, db_session):
        salesperson = UserDBFactory(role=Roles.SALES)
        result = CustomerService.get_all_by_salesperson(salesperson, db_session)
        assert result == []
...
class TestUpdate:
    def test_updates_first_name(self, db_session):
        customer = CustomerDBFactory(first_name="Old")
        data = CustomerUpdateInput(first_name="New")
    ...
    def test_partial_update_does_not_affect_other_fields(self, db_session):
        customer = CustomerDBFactory(first_name="John", last_name="Doe")
        data = CustomerUpdateInput(first_name="Jane")
        updated = CustomerService.update(customer, data, db_session)
...
class TestDelete:
    def test_deletes_customer(self, db_session):
    ...
    def test_deletes_orphan_company(self, db_session):
        customer = CustomerDBFactory()
        company_vat = customer.company_vat
        CustomerService.delete(customer, db_session)
        assert CompanyService.get_by_vat(company_vat, db_session) is None

    def test_keeps_company_when_other_customers_exist(self, db_session):
