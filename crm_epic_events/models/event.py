...
class Event(Base):
    """
    Represents an event organised for a customer under a signed contract.

    Each event is tied to exactly one `Contract` (one-to-one, enforced by a unique constraint)
    and one `Customer`. A support `User` may optionally be assigned; events without a support
    contact can be filtered via ``get_all_without_support()``.
    Deleting a linked contract or customer is restricted at the database level while the event exists.
    """
